...
RUN cd /tmp
RUN curl -O http://mirrors.estointernet.in/apache/tomcat/tomcat-8/v8.5.38/bin/apache-tomcat-8.5.38.tar.gz
RUN mkdir /opt/tomcat
RUN gzip -V
RUN tar -xvzf apache-tomcat-8.5.38.tar.gz -C /opt/tomcat --strip-components=1
RUN cd /opt/tomcat
